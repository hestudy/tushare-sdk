...
- [ ] T001 检查现有的 `.github/workflows/publish.yml` 文件,了解当前 SDK 发布流程
- [ ] T002 验证 `packages/tushare-sdk/package.json` 和 `apps/tushare-mcp/package.json` 配置
- [ ] T003 [P] 验证 GitHub Secrets 配置(NPM_AUTOMATION_TOKEN, TUSHARE_TOKEN)
...
- [ ] T004 在 `.github/workflows/publish.yml` 中添加包识别逻辑,支持标签前缀解析(sdk-v* vs mcp-v*)
- [ ] T005 修改 workflow 触发器,支持 `(sdk|mcp)-v*` 标签格式
- [ ] T006 创建 detect-package job,输出 package_id, package_name, package_path, version 变量
- [ ] T007 实现标签格式验证逻辑,确保标签符合 `^(sdk|mcp)-v\d+\.\d+\.\d+(-.+)?$` 正则
- [ ] T008 配置工作流权限声明(contents: write, id-token: write)
...
- [ ] T009 [US1] 修改 test-and-build job,使其支持多包构建验证(保持 workspace 级别命令)
- [ ] T010 [US1] 在 test-and-build job 中添加构建产物验证步骤,检查 `${{ package_path }}/dist` 目录存在
- [ ] T011 [US1] 创建 publish job,接收 detect-package 的输出变量
- [ ] T012 [US1] 在 publish job 中实现版本同步逻辑,将标签版本写入 `package.json`
- [ ] T013 [US1] 实现 dist-tag 推断逻辑(稳定版→latest,预发布版→提取标识符)
- [ ] T014 [US1] 添加 npm 发布步骤,使用 `pnpm publish --tag $DIST_TAG --no-git-checks --access public`
- [ ] T015 [US1] 配置 npm 发布的环境变量(NODE_AUTH_TOKEN, NPM_CONFIG_PROVENANCE=true)
- [ ] T016 [US1] 添加发布验证步骤,确认包已成功发布到 npm
- [ ] T017 [US1] 添加发布通知输出,包含 npm 包链接和版本信息
...
- [ ] T018 [US2] 在 publish job 的版本同步步骤后添加版本一致性验证逻辑
- [ ] T019 [US2] 实现版本一致性检查,对比标签版本和 package.json 中的版本
- [ ] T020 [US2] 添加版本冲突检测步骤,使用 `npm view` 检查 npm 上是否已存在相同版本
- [ ] T021 [US2] 配置错误处理,版本不一致或冲突时终止流程并输出清晰错误信息
- [ ] T022 [US2] 添加错误通知,提示维护者使用正确的版本号
...
- [ ] T023 [US3] 创建 create-release job,依赖 publish job 成功完成
- [ ] T024 [US3] 实现获取同包上一个标签的逻辑(使用 `git tag -l "${PACKAGE_ID}-v*" --sort=-version:refname`)
- [ ] T025 [US3] 实现变更日志生成逻辑,使用 `git log` 获取从上一版本到当前版本的所有 commits
- [ ] T026 [US3] 处理首次发布场景,检测无历史标签时显示 "First Release 🎉"
- [ ] T027 [US3] 实现 prerelease 标记判断逻辑(版本号包含 `-` 则标记为预发布)
- [ ] T028 [US3] 使用 `actions/create-release@v1` 创建 GitHub Release
- [ ] T029 [US3] 配置 Release 内容,包含标签名、变更日志、prerelease 标记
- [ ] T030 [US3] 在 Release 描述中添加 npm 包链接和文档链接
- [ ] T031 [US3] 配置 create-release job 的错误处理(失败不影响整体成功)
...
- [ ] T032 [P] 配置并发控制策略(`concurrency: group: publish-${{ github.ref }}, cancel-in-progress: false`)
- [ ] T033 [P] 添加工作流日志输出,每个关键步骤输出清晰的状态通知
- [ ] T034 [P] 优化错误消息,确保各种失败场景都有清晰的错误提示
- [ ] T035 [P] 在工作流文件中添加注释,说明每个 job 和 step 的目的
- [ ] T036 [P] 更新项目文档,说明如何使用新的标签格式发布 SDK 和 MCP 包
- [ ] T037 验证 quickstart.md 中的所有场景(稳定版、预发布版、首次发布、并发发布)
