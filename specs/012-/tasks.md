...
- [ ] T033 [P] [US2] 编写参数说明章节 - 创建参数表格结构
- [ ] T034 [P] [US2] 添加 `ts_code` 参数说明(股票代码,格式要求,示例)
- [ ] T035 [P] [US2] 添加 `period` 参数说明(报告期,YYYYMMDD格式,优先级最高)
- [ ] T036 [P] [US2] 添加 `start_date` / `end_date` 参数说明(报告期区间查询)
- [ ] T037 [P] [US2] 添加 `ann_date` 参数说明(公告日期)
- [ ] T038 [P] [US2] 添加 `report_type` 参数说明(报表类型: 1-合并报表, 2-单季合并等)
- [ ] T039 [P] [US2] 添加 `comp_type` 参数说明(公司类型)

### 完善返回字段说明

- [ ] T040 [US2] 编写返回值说明章节 - 说明返回类型 `Promise<CashFlowItem[]>`
- [ ] T041 [US2] 创建返回字段列表结构(按三大活动分类)
- [ ] T042 [P] [US2] 添加基本标识字段说明(ts_code, ann_date, end_date, report_type, comp_type等8个字段)
- [ ] T043 [P] [US2] 添加经营活动现金流字段说明(n_cashflow_act等16个核心字段,标注核心)
- [ ] T044 [P] [US2] 添加投资活动现金流字段说明(n_cashflow_inv_act等9个核心字段)
- [ ] T045 [P] [US2] 添加筹资活动现金流字段说明(n_cash_flows_fnc_act等8个核心字段)
- [ ] T046 [P] [US2] 添加现金汇总指标字段说明(n_incr_cash_cash_equ等7个核心字段)
- [ ] T047 [P] [US2] 添加补充项目字段说明(39个补充字段,可分批添加)

### 验证 User Story 2

- [ ] T048 [US2] 运行参数完整性验证测试(验证至少5个参数)
- [ ] T049 [US2] 运行字段完整性验证测试(验证至少50个核心字段与SDK模型一致)
- [ ] T050 [US2] 验证参数表格和字段表格格式正确
...
- [ ] T051 [P] [US3] 编写场景示例2: 现金流健康度分析(分析经营、投资、筹资三大活动的健康度)
- [ ] T052 [P] [US3] 编写场景示例3: 自由现金流计算(FCF = 经营现金流 - 资本支出)
- [ ] T053 [P] [US3] 编写场景示例4(可选): 多期现金流趋势分析(查询多个报告期并计算变化趋势)
- [ ] T054 [US3] 为每个场景示例添加清晰的注释和业务说明
- [ ] T055 [US3] 验证所有场景示例代码语法正确且可执行

### 验证 User Story 3

- [ ] T056 [US3] 运行代码示例验证测试(验证至少2个示例,语法正确)
- [ ] T057 [US3] 验证示例使用真实股票代码和合理时间参数
- [ ] T058 [US3] 手动验证示例代码的业务逻辑正确性
...
- [ ] T059 [P] 编写异常说明章节 - 创建异常表格(ApiError, ValidationError, NotFoundError等)
- [ ] T060 [P] 编写注意事项章节 - 至少5条重要提示
- [ ] T061 [P] 添加金额单位说明(元)
- [ ] T062 [P] 添加日期格式说明(YYYYMMDD)
- [ ] T063 [P] 添加报告期格式说明(季度末日期: 0331, 0630, 0930, 1231)
- [ ] T064 [P] 添加权限要求说明(财务数据需要至少2000积分)
- [ ] T065 [P] 添加数据时效性说明(公告后T+1日更新)

### 添加相关 API 链接

- [ ] T066 [P] 添加相关 API 章节(可选)
- [ ] T067 [P] 添加指向利润表文档的链接 (`/api/finance/income`)
- [ ] T068 [P] 添加指向资产负债表文档的链接 (`/api/finance/balance`)

### 最终验证

- [ ] T069 运行所有 E2E 导航测试,确保全部通过
- [ ] T070 运行所有文档内容验证测试,确保全部通过
- [ ] T071 验证文档与 SDK 实际实现一致性(函数签名、参数类型、返回字段)
- [ ] T072 本地构建文档站验证构建成功 (`npm run docs:build`)
- [ ] T073 本地预览验证用户体验 (`npm run docs:dev` 手动检查)
- [ ] T074 代码审查和格式化检查
